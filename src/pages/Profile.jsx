...
import {deviceWidthEnum, formatDate, formatISOWithTimezone, getSocialAccountHandle} from "../helpers/utils";
import {getProfile} from "./loaders/appLoaders";
import {useDeviceSize} from "../hooks/useDeviceSize";
import {BasicGridAppBoxes} from "../components/AppBoxes";
import NavBar from "../components/NavBar";
import useTokenData from "../hooks/useTokenData";
import {brandColorMap, timeLineCategoryMapping} from "../helpers/constants";
...
const SlowComponent = (props) => {
    // use(waitFor(props.time));   // use is a new hook for react 19.
    return <div>Slow Component {props.time}</div>;
}
...
export const PersonalDetailsSection = ({userData, showArrow, linkUrl}) => {
    return (
        <PageSectionContainer>
            <HeaderLink linkUrl={linkUrl} headerTitle={"Personal Details"} showArrow={showArrow}
                        headerClasses={"lg:text-5xl"}/>
            <PageSectionBody classes={"space-y-4 px-4"}>
                ...
                <ListContentContainer
                    ...
                    body={<a href={`tel:${userData?.phone_no}`}
                             className={"font-bold group-hover:underline"}>{userData?.phone_no || "-"}</a>}
                >
                </ListContentContainer>
        ...
        <PageSectionContainer>
            {
                userId === tokenData.tokenData?.user_id
                    ? <HeaderLink linkUrl={`/user/${tokenData.tokenData?.user_id}/apps` || linkUrl}
                                  headerTitle={"Your Apps"} showArrow={showArrow}/>
                    : <HeaderLink linkUrl={"apps" || linkUrl}
                                  headerTitle={userId === tokenData.tokenData?.user_id ? "Your Apps" : `${userData?.firstname} Apps`}
                                  showArrow={showArrow}/>
            }
                ...
                <section className={"flex flex-row flex-nowrap justify-start items-start pad-x2 py-4 overflow-x-auto"}>
                    {
                        userApps?.results?.length > 0
                            ...
                            : <div className={"mx-auto my-2 p-4 text-lg text-center font-semibold color-gray"}>No apps
                                yet</div>
                    }
                </section>
                {
    ...
    return (
        <div
            ...
            <Avatar
                src={userData?.dp}
                alt={"profile"}
                classes={"w-24 size-24 leading-[96px] md:size-40 leading-[160px] lg:size-40 lg:leading-[160px] font-bold text-xl lg:text-3xl bg-gray-200 dark:bg-base-300"}
            >
                {
                    !userData?.dp &&
                            ...
                            if (userData?.social_account_dict[socialSite]) {
                                return (
                                    <div key={index} className={"l:w-32"}>
                                        ...
                                        >
                                            <div className={"lg:tooltip lg:tooltip-bottom lg:tooltip-info"}
...
const ProfileBannerAbstractCard = ({userData, userApps}) => {
    return (
        <section
            ...
            <div
                className={"card card-bordered card-normal bg-white max-w-2xl shadow-xl -rotate-6 -top-2 opacity-60 dark:bg-base-200"}>
                <div className={"card-body gap-y-6"}>
                    ...
                    <div className="space-x-6 space-y-3">
                        {
                            "python,javascript,typescript,rust".split(",").map((userProgLang) => (
                                <span
                                    ...
                                    <span
                                        className="w-1.5 h-1.5 inline-block rounded-full bg-blue-800 dark:bg-blue-500"></span>
                                    {userProgLang}
                                </span>
                            ))
                        }
    ...
    return (
        <section
            ...
            <section
                ...
                {
                    !removeSocial
                    && <ProfileBasicSocialListCard userData={userData} socialSitesList={socialSitesList}
    ...
    return (
        <section className={"profile-container relative bg-light lg:bg-white-solid dark:bg-111314"}>
            ...
            }
            {
                ...
                size.windowWidth < deviceWidthEnum.laptop
                    ? <PageHeaderLink
                        headerTitle={userId === tokenData.tokenData?.user_id ? "My profile" : `${userData?.firstname} Page`}
                        fixTop={true}>
                        {
                            userId === tokenData.tokenData?.user_id
                                ? <Link to={"/profile/update"}
            ...
            <section className={"flex flex-col lg:flex-row lg:gap-x-12"}>
                <section className={"relative w-full lg:w-[56%] lg:mx-16 lg:top-[40px]"}>
                    ...
                    {/* Your Apps section */}
                    <AppsSection userData={userData} userApps={userApps} userId={userId} showArrow={true}
                                 linkUrl={userId === tokenData.tokenData?.user_id ? `/user/${tokenData.tokenData?.user_id}/apps` : "apps"}/>
                                ...
                                <div>
                                    {
                                        transformedTimelineGroupedByDateRef.current.map((eachTimelineItem, index) => (
                                            <>
                                                <div key={index} className="ps-2 my-2 first:mt-0">
            ...
            <section
                ...
                <header
                    className={"font-15 px-4 py-4 font-medium lg:text-lg lg:py-8 lg:font-18|pad-y4 dark:color-whitesmoke"}>{"Personal details"}</header>
                ...
                <section
                    ...
                    <div className={""}>
                        <span className={"lg:font-14"}><span
            ...
            <section
                ...
                {
                    userId === tokenData.tokenData?.user_id
                        ? <Link to={`/user/${tokenData.tokenData?.user_id}/apps`}
